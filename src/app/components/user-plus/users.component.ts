...
import { getRows, getView } from "@app/store/user/user.selectors";
...
export class UsersComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.bindData();
    this.loadList$ = this.store.select(getRows).subscribe((rows: User[]) => {
      this.rows = rows;
    });
    this.viewSub$ = this.store.select(getView).subscribe((vType: number) => {
      this.vType = vType;
    });
  }
